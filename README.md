...
The deadline for Homework 6 is Monday, November 1, 4pm. The late submission deadline is Monday, November 8, 4pm.

Refer to the homework handout [`hw06.pdf`](hw06.pdf) for details about the assignment. This file provides some information to help you get started with setting up your development environment for the homework assignments.
...
available at
`https://github.com/nyu-popl-fa21/hw06-<YOUR-GITHUB-USERNAME>`.
Note that this may take a few minutes.

* Open a browser at `https://github.com/nyu-popl-fa21/hw06-<YOUR-GITHUB-USERNAME>` with your Github username inserted at the appropriate place in the URL.
* Choose a place on your computer for your homework assignments to reside and open a terminal to that location.
* Execute the following git command: <br/>
  ```bash
  git clone https://github.com/nyu-popl-fa21/hw06-<YOUR-GITHUB-USERNAME>.git hw06
  cd hw06
  ```
...
The problems that you you should solve for this assignment are described in the file `hw06.pdf`.

The code template for solving this assignment is provided in the file 
```
src/main/scala/popl/hw06.scala 
``` 

relative to the root directory of the repository. Follow the
instructions in the [in-class-code](https://github.com/nyu-popl-fa21/in-class-code) repository to import the project into InteliJ (or use your other
favorite IDE or editor to work on the assignment).

You can implement the solutions by replacing the `???`  expressions in
`hw06.scala` with your code for the corresponding part. Please do not
...
The parsed expression is then passed to your implementation of the interpreter (the function `eval` defined in `hw06.scala`). For instance, to run your interpreter on the file `testjs/test01_arith.js`, you can execute the following command from the command line:
...
```
sbt:hw06> run testjs/test01_arith.js
```
...
Alternatively, you can run the interpreter directly from inside the IDE. To do so, right-click the file `hw06.scala` and select "More Run/Debug" and then "Modify Run Configuration". You can provide the command line arguments to the interpreter in the text field labeled "Program arguments". Enter e.g. "testjs/test01_arith.js" in this field and click "OK". When you now right-click on the file `hw06.scala` and select "Run 'hw06'", then the interpreter will be executed with the specified command line arguments.
