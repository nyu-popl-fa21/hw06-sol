  ...
  def eval(env: Env, e: Expr): Val = {
    ...
    e match {
      ...
      case BinOp(And, e1, e2) =>
        val v1 = eToVal(e1)
        if (toBool(v1)) /* EvalAndTrue */ eToVal(e2)
        else /* EvalAndFalse */ v1

      // EvalOr*
      case BinOp(Or, e1, e2) =>
        val v1 = eToVal(e1)
        if (toBool(v1)) /* EvalOrTrue */ v1 else /* EvalOrFalse */ eToVal(e2)
      ...
      // EvalIf*
      case If(e1, e2, e3) =>
        if (eToBool(e1)) /* EvalIfThen */ eToVal(e2)
  ...
  def step(e: Expr): Expr = {
    e match {
      ...
      case UnOp(UMinus, v: Val) =>
        if (debug) println("Applying rule DoUMinus")
        Num(-toNum(v))
      ...
      // DoNot
      case UnOp(Not, v: Val) =>
        if (debug) println("Applying rule DoNot")
        Bool(!toBool(v))
      ...
      case BinOp(Plus, Str(s1), v2: Val) =>
        if (debug) println("Applying rule DoPlusStr1")
        Str(s1 + toStr(v2))

      // DoPlusStr2
      case BinOp(Plus, v1: Val, Str(s2)) =>
        if (debug) println("Applying rule DoPlusStr2")
        Str(toStr(v1) + s2)

      // DoArith
      case BinOp(bop@(Plus | Minus | Times | Div), v1: Val, v2: Val) =>
        if (debug) println("Applying rule DoArith")
      ...
      case BinOp(bop@(Lt | Le | Gt | Ge), v1: Val, v2: Val) =>
        if (debug) println("Applying rule DoInequal*")
        Bool(inequalityVal(bop, v1, v2))

      // TypeErrorEqual1
      case BinOp(Eq | Ne, Function(_, _, _), _) =>
        if (debug) println("Applying rule TypeErrorEqual1")
        throw DynamicTypeError(e)

      // TypeErrorEqual2
      case BinOp(Eq | Ne, _, Function(_, _, _)) =>
        if (debug) println("Applying rule TypeErrorEqual2")
        throw DynamicTypeError(e)

      // DoEqual(===)
      case BinOp(Eq, v1: Val, v2: Val) =>
        if (debug) println("Applying rule DoEqual(===)")
        Bool(v1 == v2)
      ...
      case BinOp(And, v1: Val, e2) =>
        if (toBool(v1)) {
          if (debug) println("Applying rule DoAndTrue")
          e2
        } else {
          if (debug) println("Applying rule DoAndFalse")
          v1
        }
      ...
      case BinOp(Or, v1: Val, e2) =>
        if (toBool(v1)) {
          if (debug) println("Applying rule DoOrTrue")
          v1
        } else {
          if (debug) println("Applying rule DoOrFalse")
          e2
        }

      // DoConstDecl
      case ConstDecl(x, v1: Val, e2) =>
        if (debug) println("Applying rule DoConst")
        subst(e2, x, v1)

      // DoIf*
      case If(v1: Val, e2, e3) =>
        if (toBool(v1)) {
          if (debug) println("Applying rule DoIfThen")
          e2
        } else {
          if (debug) println("Applying rule DoIfElse")
          e3
        }

      // TypeErrorCall
      case Call(Num(_) | Bool(_) | Str(_) | Undefined, _) =>
        if (debug) println("Applying rule TypeErrorCall")
        throw DynamicTypeError(e)
      ...
      case Call(v1@Function(p, x2, e1), v2: Val) =>
        p match {
          case None =>
            if (debug) println("Applying rule DoCall")
            subst(e1, x2, v2)
          case Some(x1) =>
            if (debug) println("Applying rule DoCallRec")
      ...
      // SearchPrint
      case Print(e) =>
        if (debug) println("Applying rule SearchPrint")
        Print(step(e))

      // SearchUop
      case UnOp(uop, e) =>
        if (debug) println("Applying rule SearchUop")
        UnOp(uop, step(e))
      ...
      // SearchBop1
      case BinOp(bop, e1, e2) =>
        if (debug) println("Applying rule SearchBop1")
        BinOp(bop, step(e1), e2)

      // SearchIf
      case If(e1, e2, e3) =>
        if (debug) println("Applying rule SearchIf")
        If(step(e1), e2, e3)

      // SearchConst
      case ConstDecl(x, e1, e2) =>
        if (debug) println("Applying rule SearchConst")
        ConstDecl(x, step(e1), e2)

      // SearchCall2
      case Call(v1: Val, e2) =>
        if (debug) println("Applying rule SearchCall2")
        Call(v1, step(e2))

      // SearchCall1
      case Call(e1, e2) =>
        if (debug) println("Applying rule SearchCall1")
        Call(step(e1), e2)
