...
class hw06SpecSmall extends AnyFlatSpec {

  "subst" should "only replace free occurrences of x" in {
    val x = "x"
    val y = "y"
    val z = "z"
    val e1 = BinOp(Plus, Var(x), Var(y))
    val e2 = BinOp(Plus, Num(3), Var(y))
    assert(subst(e1, x, Num(3)) === e2)
    assert(subst(ConstDecl(y, e1, e1), x, Num(3)) === ConstDecl(y, e2, e2))
    assert(subst(ConstDecl(x, e1, e1), x, Num(3)) === ConstDecl(x, e2, e1))
    assert(subst(Function(None, y, e1), x, Num(3)) === Function(None, y, e2))
    assert(subst(Function(None, x, e1), x, Num(3)) === Function(None, x, e1))
    assert(subst(Function(Some(z), y, e1), x, Num(3)) === Function(Some(z), y, e2))
    assert(subst(Function(Some(x), y, e1), x, Num(3)) === Function(Some(x), y, e1))
  } 
